...
5. Building on the previous problem, write a <q>C</q> function <code>makeNBO.c</code> that reads a number from the command line arguments and swaps its byte order, if necessary, to be in Network Byte Order.  Create a test program to call your function and display the results on stdout.
6. Finally, create a <strong>nasm</strong> program called <code>makeNBO.nasm</code> which does the byte order swapping BY CALLING YOUR <q>C</q> function.  Make your <strong>nasm</strong> program call the function <i><u>at least</u></i> five times with different values to test it.
